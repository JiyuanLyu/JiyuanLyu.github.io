...
def get_message_db():
    ...
    cmd = "CREATE TABLE IF NOT EXISTS messages (id INTEGER NOT NULL AUTOINCREMENT, handle TEXT, message TEXT)"
    cursor.execute(cmd)
...
Great! Then define a `submit()` function to transmit and receive the input data. Make sure that it supports both `POST` and `GET` methods and render the `submit.html` template.
...
Write a new template called `view.html`, extending `base.html` template, to display the messages extracted from `random_messages()`.
...
Define a function for user to view messages in the database. After grabing 10 random messages with `random_messages()` (will be defined in Part 2), pass the messages to `render_template()`.
...
In this part, I edit a css file `style.css` to customize my WebApp.

The file can be accessed in the GitHub repository link at the beginning of this blog post. 
